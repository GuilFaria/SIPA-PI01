...
def tela_login():
    ...
    with st.form(key="login e senha", enter_to_submit=False):
        ...
        if submit_button:
            ...
            login_enc = encriptografa_sha3_256(login)
            password_enc = encriptografa_sha3_256(senha)
            
            st.write(login_enc, password_enc)
            if user_auth(login_enc, password_enc):
...
def user_auth(login_enc, password_enc):
    ...
    cur.execute('''SELECT * FROM usuarios us 
                WHERE 1=1
                AND us.login = %s 
                AND us.senha = %s''', 
